...
abstract contract Whitelist is Ownable2Step {
    ...
    function viewWhitelisted(uint256 cursor, uint256 size) external view returns (TokenInfo[] memory, uint256) {
        if (cursor >= whitelistedTokens.length) {
            return (new TokenInfo[](0), cursor);
        }
        uint256 length = size;
...
contract AgoraX is Ownable2Step, ERC20, ReentrancyGuard, Whitelist {
    ...
    /// @notice Initializes the contract with fee settings and limits
    constructor(
        address _feeAddress,
        uint256 _listingFee,
        ...
        require(_protocolFeesLimit > 0, "AgoraX: Protocol fee limit must be > 0");
    ...
    function updateFeeAddress(address _newFeeAddress) external onlyOwner {
        require(_newFeeAddress != address(0) && _newFeeAddress != address(this), "AgoraX: Invalid fee address");
        require(_newFeeAddress != feeAddress, "AgoraX: New fee address matches current");
    ...
    /// @dev Only callable by the owner
    function updateCooldownPeriod(uint64 _newCooldownPeriod) external onlyOwner {
    ...
    /// @dev Only callable by the owner
    function cleanInactiveUsers(uint256 cursor, uint256 size) external onlyOwner returns (uint256 nextCursor) {
    ...
    function placeOrder(
        ...
        whenNotPaused 
    {
        require(_buyTokensIndex.length > 0, "AgoraX: At least one buy token required");
        require(_buyTokensIndex.length <= 50, "AgoraX: Too many buy tokens");
        require(_buyTokensIndex.length == _buyAmounts.length, "AgoraX: Array length mismatch");
        ...
        require(_expirationTime > block.timestamp, "AgoraX: Expiration must be in the future");
        ...
        if (_sellToken == NATIVE_ADDRESS) {
            require(remainingValue >= _sellAmount, "AgoraX: Insufficient PLS message value");
        ...
        emit OrderPlaced(msg.sender, orderCounter, _sellToken, _sellAmount, _buyTokensIndex, _buyAmounts, _expirationTime, _allOrNothing);
    }
    ...
    function cancelOrder(uint256 _orderID, address _recipient) external nonReentrant validOrderID(_orderID) {
        require(_recipient != address(0) && _recipient != address(this), "AgoraX: Invalid recipient");
        UserOrderDetails memory userOrderDetails = userDetailsByOrderID[_orderID];
        ...
        uint256 redeemableSell = orderDetailsWithID.orderDetails.sellAmount - remainingSellAmount - orderDetailsWithID.redeemedSellAmount;
        if (redeemableSell > 0) {
        ...
        if (orderDetailsWithID.orderDetails.sellToken == NATIVE_ADDRESS) {
            ...
            IERC20(orderDetailsWithID.orderDetails.sellToken).safeTransfer(msg.sender, remainingSellAmount);
        }
    ...
    function cancelAllExpiredOrders(address _recipient) external nonReentrant returns (uint256[] memory cancelledOrderIDs) {
        require(_recipient != address(0) && _recipient != address(this), "AgoraX: Invalid recipient");
        ...
        for (uint256 i = 0; i < userOrdersLength; ++i) {
            ...
            if (order.status == OrderStatus.Active && 
                ...
                uint256 redeemableSell = order.orderDetails.sellAmount - order.remainingSellAmount - order.redeemedSellAmount;
                if (redeemableSell > 0) {
                ...
                if (order.orderDetails.sellToken == NATIVE_ADDRESS) {
                    ...
                    IERC20(order.orderDetails.sellToken).safeTransfer(msg.sender, order.remainingSellAmount);
                }
    ...
    /// @notice Collects proceeds for a specific buy token in an order
    /// @param _orderID The ID of the order
    /// @param _buyTokenIndexInOrder Index of the buy token in the order's buyTokensIndex array
    /// @param _recipient Address to send proceeds to (recipient must be able to accept PLS proceeds, if any)
    function collectProceedsByToken(uint256 _orderID, uint256 _buyTokenIndexInOrder, address _recipient) external nonReentrant validOrderID(_orderID) {
        require(_recipient != address(0) && _recipient != address(this), "AgoraX: Invalid recipient");
        UserOrderDetails memory userOrderDetails = userDetailsByOrderID[_orderID];
        require(userOrderDetails.orderOwner == msg.sender, "AgoraX: Unauthorized");
        
        OrderDetailsWithID storage orderDetailsWithID = orders[msg.sender][userOrderDetails.orderIndex];
        uint256 buyTokenIndex = orderDetailsWithID.orderDetails.buyTokensIndex[_buyTokenIndexInOrder];
        (address buyToken, ) = getTokenInfoAt(buyTokenIndex);
        
        uint256 boughtAmount = buyTransactionsByOrderID[_orderID][buyTokenIndex];
        uint256 soldForThisToken = soldByBuyToken[_orderID][buyTokenIndex];
        require(boughtAmount > 0 && soldForThisToken > 0, "AgoraX: Nothing to collect for this token");
        ...
        if (buyToken == NATIVE_ADDRESS) {
            ...
            if (success) {
                if (returndata.length == 0) { // Assumes success for older tokens that have no return data
                } else if (returndata.length == 32) {
                    if (!abi.decode(returndata, (bool))) {
                        success = false;
                    }
                } else {
                    success = false; // Rejects tokens with malformed return data
                }
            }
        }

        if (success) {
            buyTransactionsByOrderID[_orderID][buyTokenIndex] = 0;
            soldByBuyToken[_orderID][buyTokenIndex] = 0;
            _burn(msg.sender, soldForThisToken);
            orderDetailsWithID.redeemedSellAmount += soldForThisToken;
            
            uint256[] memory indices = new uint256[](1);
            uint256[] memory amounts = new uint256[](1);
            indices[0] = _buyTokenIndexInOrder;
            amounts[0] = boughtAmount;
            emit OrderProceedsCollected(msg.sender, _orderID, indices, amounts);
        } else {
    ...
    function viewCollectableProceeds(uint256 _orderID)
        external
        view
        ...
        )
    {
        ...
        OrderDetailsWithID memory order = orders[userDetails.orderOwner][userDetails.orderIndex];
        ...
        for (uint256 i = 0; i < len; ++i) {
            ...
            uint256 accumulated = buyTransactionsByOrderID[_orderID][tokenIndex];
    ...
    function updateOrderExpiration(uint256 _orderID, uint64 _newExpiration) external nonReentrant validOrderID(_orderID) {
        UserOrderDetails memory userDetails = userDetailsByOrderID[_orderID];
    ...
    function findFillableOrders(
        ...
        require(_sellToken != address(0) && _sellToken != address(this), "AgoraX: Invalid sell token");
        ...
        // Out-of-bounds check for graceful handling of an invalid cursor
        if (cursor >= totalUsers) {
            return (new uint256[](0), cursor);
        }

        uint256 startUser = cursor;
        ...
        // Bubble sort ascending if more than one matching order is found
        if (cnt > 1) {
    ...
    function _fillOrder(
        ...
        uint256 buyTokenIndex;
        {
            ...
            require(_soldAmount <= orderDetailsWithID.remainingSellAmount, "AgoraX: Exceeds order availability");
    ...
    function _collectProceeds(uint256 _orderID, address _recipient) internal {
        ...
        uint256 len = orderDetailsWithID.orderDetails.buyTokensIndex.length;
        uint256[] memory collectedIndices = new uint256[](len);
        uint256[] memory collectedAmounts = new uint256[](len);
        uint256 collectedCount = 0;
        uint256 totalRedeemedThisCall = 0;

        for (uint256 i = 0; i < len; ++i) {
            uint256 buyTokenIndex = orderDetailsWithID.orderDetails.buyTokensIndex[i];
            (address buyToken, ) = getTokenInfoAt(buyTokenIndex);
            uint256 boughtAmount = buyTransactionsByOrderID[_orderID][buyTokenIndex];
            uint256 soldForThisToken = soldByBuyToken[_orderID][buyTokenIndex];
            
            if (boughtAmount == 0 || soldForThisToken == 0) continue;
            ...
            if (success) {
                ...
                collectedIndices[collectedCount] = i;
                collectedAmounts[collectedCount] = boughtAmount;
                collectedCount++;
            } else {
                emit ProceedsCollectionFailed(msg.sender, _orderID, buyToken);
            }
        ...
        if (totalRedeemedThisCall > 0) {
            _burn(msg.sender, totalRedeemedThisCall);
            orderDetailsWithID.redeemedSellAmount += totalRedeemedThisCall;
            
            // Shrink arrays for event
            uint256[] memory finalIndices = new uint256[](collectedCount);
            uint256[] memory finalAmounts = new uint256[](collectedCount);
            for (uint256 j = 0; j < collectedCount; j++) {
                finalIndices[j] = collectedIndices[j];
                finalAmounts[j] = collectedAmounts[j];
            }
            
            emit OrderProceedsCollected(msg.sender, _orderID, finalIndices, finalAmounts);
        }
    }
    ...
    function trackedUserCounts() external view returns (uint256 totalUsers, uint256 inactiveUsers) {
    ...
    // ========== FALLBACK ==========
    /// @dev Fallback function to reject accidental direct-to-contract PLS transfers
